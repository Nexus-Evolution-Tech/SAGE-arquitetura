...
A issue define o recorte da pergunta; a spec normativa define o desenho de projeção: `qr_code` e
`cartao_rfid` permanecem em `Pessoa.leitura`, sob autorização existente, sem serem tratados como
`segredo` nesta release. A evidência dos fluxos não é indistinta: há endpoint e auditoria explícita
para a emissão de QR; para RFID, há atribuição dentro de operação auditada de escrita, sem emissor
ou trilha específica de emissão comprovados. A matriz abaixo torna essa decisão verificável por
operação, sem ampliar o contrato.
...
- `src/controllers/peopleController.js:246-264`: a geração de QR atualiza Pessoa e devolve o
  campo na resposta.
- `src/controllers/peopleController.js:132-160` e `src/services/peopleService.js:110-167`: o
  `PATCH /pessoas/:id` escreve Pessoa dentro de operação auditada; o cadastro `POST /pessoas`
  legado também aceita o fluxo de atribuição do campo.
...
legítimos de leitura/retorno. Para QR, a evidência também mostra endpoint e emissão explicitamente
auditados. Para RFID, a evidência mostra atribuição dentro da operação auditada de escrita em
Pessoa, mas não emissor ou auditoria específica de emissão. `Pessoa.senha_acesso` continua sendo
o segredo derivado do DDL. Essa classificação reconhece o risco; não o elimina, não cria
armazenamento seguro e não transforma a resposta de negócio em log. Valores reais, credenciais,
dados pessoais e logs não fazem parte desta spec.
...
| `cartao_rfid` | emissão | Há atribuição em operação auditada de escrita no `PATCH /pessoas/:id`; isso não equivale a emissão auditada específica. O cadastro `POST /pessoas` legado também existe. Não há emissor ou trilha específica de emissão comprovados. | Cadastro/edição de Pessoa que aceita `cartao_rfid` em `Pessoa.escrita`, alcançado por `POST /pessoas` e `PATCH /pessoas/:id`. | `exige('SECRETARIA')`, com `ADMINISTRADOR` também permitido pela hierarquia atual, e as pré-condições já existentes de escrita. Nenhuma elevação para ADMIN-only. | O `PATCH` permanece em `executarOperacaoAuditada`/`REGISTRO_EDITADO`: isso audita a operação de escrita, não a emissão RFID como evento distinto. Para o `POST` legado, não se infere trilha específica. O valor não pode aparecer em detalhe ou log. | **Dentro:** preservar o campo e o fluxo autorizado. **Fora:** afirmar emissão auditada, definir emissor ou nova trilha específica. |
...
4. A atribuição de `cartao_rfid` dentro da operação auditada de escrita no `PATCH` permanece; isso
   não declara auditoria específica de emissão RFID, e o `POST` legado não recebe política nova.
5. Não existe, nesta issue, restrição global `ADMINISTRADOR`-only. A autorização existente de
   nível `SECRETARIA` continua permitindo `SECRETARIA` e `ADMINISTRADOR`.
6. Nenhuma projeção, payload, rota, status, autorização, efeito ou auditoria do B0 é alterada.
...
operações cobertas, referências normativas e evidências reais apontadas. A verificação distingue
explicitamente QR com emissão auditada de RFID com atribuição dentro de operação auditada de
escrita; ela não alega auditoria específica de emissão RFID inexistente. Este pacote altera apenas
